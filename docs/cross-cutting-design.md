...
- 活动开始前由调度任务请求活动服务预热活动详情和库存 Redis 键；预热完成后，开始时刻的状态 CAS 才能激活活动。用户限购计数在用户参与时由 Lua 原子初始化。暂停恢复时，活动服务先预热详情再执行状态 CAS，并保留已有库存计数；仅当库存键丢失时，在互斥保护下从 PostgreSQL 当前权威值重建，禁止恢复时重置库存。
