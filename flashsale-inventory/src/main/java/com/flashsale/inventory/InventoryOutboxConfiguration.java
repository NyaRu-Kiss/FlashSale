...
final class InventoryOutboxConfiguration {
    @Bean OutboxPort inventoryOutboxPort(JdbcTemplate jdbc, ObjectMapper json, PlatformTransactionManager manager,
            @Value("${flashsale.outbox.max-attempts:8}") int maxAttempts) {
        return OutboxDispatchConfiguration.port(jdbc, json, manager, "inventory_outbox", maxAttempts);
    }
    @Bean(destroyMethod = "close") RocketMqMessageTransport inventoryOutboxTransport(@Value("${flashsale.outbox.producer-group:flashsale-inventory}") String group,
    ...
    @Bean OutboxDispatcher inventoryOutboxDispatcher(OutboxPort port, MessageTransport transport,
    ...
    @Bean OutboxDispatchTask inventoryOutboxDispatchTask(OutboxDispatcher dispatcher, MessagingMetrics metrics) { return OutboxDispatchConfiguration.task(dispatcher, metrics); }
}
