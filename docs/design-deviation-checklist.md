...
| # | 优先级 | 设计要求 | 当前实现 | 证据 |
|---|---|---|---|---|
| 1 | 高 | 使用 Spring Cloud Gateway + Sentinel，按接口、IP 和热点参数限流 | 实际使用 Spring MVC Controller 代理；限流是单机内存 IP 每秒 100 次，没有 Sentinel、接口维度或热点参数维度 | [GatewaySecurityFilter.java:23-40](../flashsale-gateway/src/main/java/com/flashsale/gateway/GatewaySecurityFilter.java#L23)；[flashsale-gateway/pom.xml:7](../flashsale-gateway/pom.xml#L7) |
| 2 | 高 | 使用 Nacos 服务注册/配置和 OpenFeign 服务调用 | Gateway 使用环境变量硬编码服务地址和 `RestClient`，未发现 Nacos/OpenFeign 运行时接入 | [UpstreamResolver.java:11-30](../flashsale-gateway/src/main/java/com/flashsale/gateway/UpstreamResolver.java#L11)；[docker-compose.yml:77](../docker-compose.yml#L77) |
| 3 | 高 | 商品和优惠券查询采用 Cache Aside、按键锁、二次检查和 TTL 抖动 | 商品、优惠券模板查询直接访问 PostgreSQL，没有 Redis 读缓存、重建锁、二次检查或随机 TTL | [ProductRepository.java:10-12](../flashsale-product/src/main/java/com/flashsale/product/ProductRepository.java#L10)；[CouponTemplateRepository.java:7-8](../flashsale-coupon/src/main/java/com/flashsale/coupon/CouponTemplateRepository.java#L7) |
| 4 | 高 | 商品、优惠券更新采用延迟双删，并写审计和 Outbox | 商品和优惠券写入只更新业务表，未发现缓存删除、运营审计或对应 Outbox 写入 | [ProductRepository.java:7-9](../flashsale-product/src/main/java/com/flashsale/product/ProductRepository.java#L7)；[CouponTemplateRepository.java:4-6](../flashsale-coupon/src/main/java/com/flashsale/coupon/CouponTemplateRepository.java#L4) |
