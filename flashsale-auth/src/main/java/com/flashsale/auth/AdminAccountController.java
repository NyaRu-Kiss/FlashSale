...
class AdminAccountController {
    ...
    @GetMapping("/audit-logs") public ApiResponse<PageResponse<AccountAuditView>> audits(@RequestHeader("Authorization") String auth,@RequestParam(defaultValue="1") int page,@RequestParam(name="page_size",defaultValue="20") int size){ Principal actor=actor(auth); var items=service.audits(actor,page,size).stream().map(this::auditView).toList(); return ApiResponse.success(new PageResponse<>(items,page,size,service.auditCount(actor)),TraceContext.getOrCreate()); }
    ...
    private AccountAuditView auditView(AdminAccountService.AccountAudit audit){ return new AccountAuditView(audit.id(),audit.operatorId(),audit.targetId(),audit.action(),audit.traceId(),audit.createdAt()); }
    ...
    record AccountAuditView(long id,long operatorId,long targetId,String action,String traceId,java.time.OffsetDateTime createdAt){}
}
