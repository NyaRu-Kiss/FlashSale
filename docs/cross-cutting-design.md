...
- 非活动商品和可领取优惠券模板采用 Cache Aside：先读 Redis，未命中时用 `SET NX PX` 获取按缓存键粒度的重建锁并二次检查；只有锁持有者可查询 PostgreSQL 和回填可见配置。未获得锁的请求短暂退避后重试缓存，不得并发穿透数据库；锁通过 Lua 校验 owner token 后释放。
- OPERATOR 更新商品或优惠券模板时采用延迟双删：先删除相关缓存；再在 PostgreSQL 事务中更新配置、审计和本服务 Outbox；事务提交后通过 RocketMQ 延时事件再次删除缓存。活动一期不允许更新配置。
- 商品下架、活动取消/暂停/结束、优惠券暂停/结束领取属于消费者视图的逻辑删除，事务提交后必须失效缓存。第一阶段不提供物理删除接口。
- 活动开始前由调度任务请求活动服务预热活动详情和库存 Redis 键；预热完成后，开始时刻的状态 CAS 才能激活活动。用户限购计数在用户参与时由 Lua 原子初始化。
- 暂停时，活动服务用 Redis Lua 原子关闭新预扣并等待暂停前在途请求完成提交或补偿；随后在 PostgreSQL 锁定 `activity_inventory_sequence`，以 `next_event_sequence - 1` 截取暂停屏障。活动库存预扣与订单取消/超时释放都必须在各自 PostgreSQL 本地事务内分配事件序号、写入 `activity_inventory_event` 和对应生产端 Outbox；事件序号只在成功提交时分配，避免补偿造成序号空洞。
- 暂停前已提交的预扣事件，以及暂停期间已有订单取消/超时产生的释放事件，继续经 Outbox/MQ 投递和消费，绝不能因活动已暂停而丢弃、拒绝或回滚。释放消费者必须以事件号幂等地回补 Redis。运营恢复请求创建异步恢复任务并截取 `recovery_barrier_sequence`；只有屏障内全部相关 Outbox 已发送、`activity_inventory_checkpoint` 连续追平、Redis/事件账本/库存流水/有效保留对账成功且活动详情已预热后，活动服务才允许 `PAUSED → ACTIVE`。任一步失败保持暂停并告警。
- 缓存失效事件必须带资源类型、资源 ID、缓存键和 Trace ID；重复删除安全，失败按 Outbox 规则重试并告警。

## 4. Outbox 与 RocketMQ 投递
