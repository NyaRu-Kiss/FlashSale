...
final class OrderOutboxConfiguration {
    @Bean OutboxPort orderOutboxPort(JdbcTemplate jdbc, ObjectMapper json, PlatformTransactionManager manager,
            @Value("${flashsale.outbox.max-attempts:8}") int maxAttempts) {
        return OutboxDispatchConfiguration.port(jdbc, json, manager, "order_outbox", maxAttempts);
    }
    @Bean(destroyMethod = "close") RocketMqMessageTransport orderOutboxTransport(@Value("${flashsale.outbox.producer-group:flashsale-order}") String group,
    ...
    @Bean OutboxDispatcher orderOutboxDispatcher(OutboxPort port, MessageTransport transport,
    ...
    @Bean OutboxDispatchTask orderOutboxDispatchTask(OutboxDispatcher dispatcher, MessagingMetrics metrics) { return OutboxDispatchConfiguration.task(dispatcher, metrics); }
}
