...
| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| H01 | 全量 HTTP 接口契约与权限验收 | C01-C03,D01-D05,E01-E06,F01-F07,G04 | Auth、商品、优惠券、活动、订单、支付、库存和运营接口逐项符合 `api-design.md`：路径/方法、请求响应字段、分页、错误码、JWT 角色、资源归属、幂等键和 Trace ID 一致；未授权、越权、重复请求和非法参数均返回规定结果 | 2026-09-25：`docker compose -f docker-compose.yml -f docker-compose.acceptance.yml --profile r21 config` 通过；验收 PostgreSQL 初始缺少 `flashsale_acceptance` 数据库，手工创建后 Flyway V1 通过。业务服务启动失败：验收镜像中多个服务实际启动 `PaymentApplication`，因缺少 `BusinessMetrics` Bean 退出，未能执行 HTTP 契约请求；服务和依赖已关闭。保持 DOING，待修复验收镜像/启动配置后重试 | DOING |
