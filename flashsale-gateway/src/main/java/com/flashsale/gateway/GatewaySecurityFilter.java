...
public class GatewaySecurityFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest req,HttpServletResponse res,FilterChain chain) throws ServletException,IOException {
        ...
        if (path.startsWith("/api/v1/admin/") && principal.role()!=Role.OPERATOR && principal.role()!=Role.ADMIN) { write(res,403,"FORBIDDEN","operator role required",trace); return; }
        req.setAttribute("principal",principal); chain.doFilter(req,res);
    }
    private boolean isPublic(String p){ return p.equals("/actuator/health")||p.startsWith("/api/v1/auth/")||p.equals("/api/v1/payments/callback"); }
