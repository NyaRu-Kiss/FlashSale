...
| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| H01 | 全量 HTTP 接口契约与权限验收 | C01-C03,D01-D05,E01-E06,F01-F07,G04 | Auth、商品、优惠券、活动、订单、支付、库存和运营接口逐项符合 `api-design.md`：路径/方法、请求响应字段、分页、错误码、JWT 角色、资源归属、幂等键和 Trace ID 一致；未授权、越权、重复请求和非法参数均返回规定结果 | 2026-09-25：使用 `docker-compose.yml` + `docker-compose.acceptance.yml --profile r21` 分层启动验收环境，Compose 配置和 Flyway V1 通过；Auth、Product、Activity、Inventory、Coupon、Order、Payment、Gateway 均健康。真实 HTTP 已通过：注册/登录/JWT/Trace、商品创建/上架/浏览/运营查询、优惠券创建/更新限制/暂停/恢复/领取/我的券、活动创建/取消/重复取消/指标、订单预览/创建/列表/详情/明细、订单幂等重复与冲突、支付成功、支付回调文档字段与回调幂等、CUSTOMER/OPERATOR/ADMIN 权限隔离、未授权/非法分页/重复注册/资源不存在、库存入口未授权与运营 JWT 校验。真实活动业务链路已通过：活动预热投影、开始、活动订单预占、支付确认库存、重复支付幂等、取消释放库存、重复取消幂等；活动库存 Redis 从 9 减至 8 后恢复至 8。活动暂停、重复暂停、恢复请求和恢复查询通过；恢复 worker 真实执行并定位到 `CHECKPOINT_GAP`，原因是验收环境未启用订单 Outbox 投递与活动消费者检查点，任务保持 `FAILED/PAUSED`。支付金额/币种异常通过。已修复并提交：验收镜像入口、独立服务注册名、JSONB Outbox 参数、状态枚举更新、活动 snake_case 字段、订单明细查询、领券 Outbox 参数、缺少幂等请求头映射为 `VALIDATION_ERROR`、支付回调兼容 `order_number/paid_at` 文档字段并校验订单归属、库存入口强制有效内部 JWT。验收峰值约 7.6 GiB；本轮启动的业务、基础设施和 XXL-Job 服务已停止，保留预先存在的独立 `redis` 容器。库存验收种子已新增并验证：`INVENTORY_ACCEPTANCE_SEED=9001=10` 仅 acceptance profile 生效；真实 HTTP 通过预占/重复预占、确认/重复确认、已确认释放错误、释放/重复释放矩阵，响应均含 Trace ID。验收峰值约 968.7 MiB（Nacos）；活动 Outbox、Activity Consumer、XXL-Job 已启用并完成恢复成功矩阵：新建当前时间窗口活动 7，预热→开始→暂停后调用恢复，任务 id=4、barrier=0、状态 `SUCCEEDED`，活动状态恢复为 `ACTIVE`；过期活动 5 的 `ACTIVITY_NOT_READY` 已确认是验收数据窗口问题。恢复相关测试与 Activity/Common、Order/Activity/Common 回归通过。H01 矩阵完成；本轮验收服务已清理，保留预先存在的独立 `redis` 容器 | DONE |
