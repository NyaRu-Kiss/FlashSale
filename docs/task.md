...
| H03 | 高并发、幂等与容量压测 | D04,E02-E03,F02-F04,G01-G04,H02 | 覆盖活动热点库存、同用户限购、优惠券领取、重复提交、支付/取消竞争和 MQ 消费；核心正确性 100% 满足 AC01-AC08，零负库存、零重复有效订单/券/支付；报告记录并发量、持续时间、吞吐、P95/P99、错误率、Outbox 积压和消费延迟，并与发布基线比较 | DOING（2026-09-26）：已添加 `load/k6/h03.js`、公共请求库和运行说明；覆盖活动 1 秒 10,000 请求突发、商品读取 5,000 VUs、普通购买 10→50→100、同用户限购、领券、重复下单、支付/取消竞争。GHCR `ghcr.io/grafana/k6:latest` 已成功拉取，k6 `inspect` 和脚本静态语法检查通过。真实业务小流量启动被既有 acceptance PostgreSQL 数据卷阻断：Flyway V1 已应用 checksum 与当前文件不一致；未执行 repair 或删除数据卷，已停止本轮 Compose 容器。实际压测和数据库/Redis/MQ 对账待使用干净测试数据卷后执行。 | DOING |
