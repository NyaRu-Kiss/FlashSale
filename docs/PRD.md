...
- **第一阶段范围：** 用户、商品、优惠券、限时活动、订单、模拟支付、超时取消、履约、运营管理、高并发测试。
...
用户、商品、优惠券、限时营销活动、活动购买、订单、模拟支付、订单超时取消、优惠券核销、库存管理、运营管理、高并发测试。
...
| 编号 | 问题 | 影响 |
|---|---|---|
| Q01 | 下单扣库存，还是支付扣库存？ | 影响超卖方案、订单超时恢复 |
| Q02 | 优惠券下单锁定，还是支付核销？ | 影响优惠券状态机、超时恢复 |
| Q03 | 订单超时时间是多少？ | 影响取消任务与用户体验 |
| Q04 | 活动暂停 / 取消后，已创建的待支付订单能否继续支付？ | 影响订单状态与活动状态联动 |
| Q05 | 重复提交订单的幂等键是什么？ | 影响接口设计与防重方案 |
| Q06 | 已取消订单是否释放限购名额？ | 影响限购统计口径 |
| Q07 | 活动结束边界按请求到达时间，还是订单创建时间判断？ | 影响活动结束瞬间并发处理 |
| Q08 | 商品库存与活动库存是什么关系？ | 影响库存模型与扣减逻辑 |
| Q09 | 支付成功但履约失败，订单应处于什么状态？ | 影响履约与异常恢复 |
| Q10 | 优惠券超时恢复后是“未使用”还是“已过期”？ | 影响优惠券状态定义 |
| Q11 | 优惠券暂停领取后能否恢复？ | 影响优惠券领取状态 |
| Q12 | 履约失败时订单处于什么状态？ | 影响订单状态与异常恢复 |
| Q13 | 订单取消后优惠券恢复为“未使用”还是“已过期”？ | 影响优惠券状态与恢复逻辑 |
| Q14 | 库存恢复的是商品库存还是活动库存？ | 影响库存恢复逻辑 |
