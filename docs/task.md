...
| C02 | ADMIN 账户管理 | C01 | 角色、启停用、最后 ADMIN 保护和审计完整 | API/事务测试（实现完成；受本机 JDK 版本阻塞） | DONE |
| C03 | Gateway 路由、鉴权与限流 | B02-B03,A03 | 路由、JWT、Sentinel、Trace 生效 | Gateway 集成测试（实现完成；受本机 JDK 版本阻塞） | DONE |

## D. 商品与优惠券

| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| D01 | Product 服务 CRUD 与上下架 | B01-B05,C01 | 商品规则、审计、缓存失效正确 | API/事务测试 | TODO |
| D02 | 商品库存与直接订单库存流水 | D01,B05 | 条件扣减、释放、不可超卖 | 并发测试 | TODO |
| D03 | Coupon 模板管理 | B01-B05,C01 | 创建、修改、暂停、恢复、审计正确 | API/事务测试 | TODO |
| D04 | 优惠券领取 Lua、幂等和 Outbox | D03,B05-B06 | 不超发、不重复领取、失败可补偿 | 并发/故障测试 | TODO |
| D05 | 用户券状态机与订单锁券/恢复 | D04 | AVAILABLE/RESERVED/CONSUMED/EXPIRED 合法迁移 | 状态机测试 | TODO |

## E. 活动与活动库存

| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| E01 | 活动创建、预热、开始、结束 | B01-B05,C01 | 活动基础状态和缓存正确 | API/任务测试 | TODO |
...
| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| F01 | 订单预览与金额快照 | D01,D05,E01 | 只校验不占资源，金额快照明确 | 单元/API 测试 | TODO |
| F02 | 普通订单创建与幂等 | D02,B05-B06,F01 | 多商品全量预留或整体失败 | 并发/幂等测试 | TODO |
| F03 | 活动订单创建与限购 | E02,E03,F01 | 单活动单商品，Redis 预扣与 DB 账本一致 | 并发/故障测试 | TODO |
| F04 | 用户取消、超时释放 | D02,D05,E02,F02-F03 | CAS 只释放一次，活动 RELEASE 可追平 | 竞争/补偿测试 | TODO |
| F05 | Payment 支付幂等与回调 | F02-F04,B05-B06 | 重复支付/回调只生效一次 | 并发/回调测试 | TODO |
| F06 | 支付成功确认与履约 | F05,D05 | 库存确认、券核销、履约、支付事件同事务 | 事务/幂等测试 | TODO |
| F07 | 延时超时取消与 Job 兜底 | F04-F06,A03 | 支付与超时竞争得到唯一合法终态 | 延时/竞争测试 | TODO |

## G. 投递、补偿与运维

| ID | 任务 | 前置 | 完成标准 | 验证 | 状态 |
|---|---|---|---|---|---|
| G01 | 各服务 Outbox 投递器 | B05,D01,D04,E02,F04,F06 | 批量、租约、退避、重复投递可用 | 集成/故障测试 | TODO |
| G02 | 各消费者幂等处理器 | B06,D04,E03,F05-F07 | 重复、失败、PROCESSING 超时可恢复 | MQ 集成测试 | TODO |
| G03 | XXL-Job 对账与补偿 | E03-E05,D04-D05,F04,F07 | Redis/DB/账本/Outbox/订单状态可对账补偿 | 故障演练 | TODO |
| G04 | 监控、指标、日志和告警 | B02,G01-G03 | 覆盖延迟、积压、库存不一致、死信、补偿失败 | 指标检查 | TODO |
