...
@Repository class ProductRepository {
 ...
 Product update(long id,Product p){return jdbc.queryForObject("update product set name=?,description=?,list_price_minor=?,available_stock=?,updated_by=?,version=version+1 where id=? returning id,sku,name,description,list_price_minor,available_stock,status,updated_by",(r,n)->map(r),p.name(),p.description(),p.priceMinor(),p.stock(),p.updatedBy(),id);}
